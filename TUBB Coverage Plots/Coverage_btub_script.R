...
vertical_bands <- data.frame(
  ...
  label = c("Glu198", "Phe200", "Ile194", "Phe168", "Truncation")
)
...
smoothed_plot <- ggplot(coverage_df, aes(x = pos, y = smoothed_coverage)) +
  ...
  labs(
    ...
    y = "Coverage",
    title = "Nanopore long read data on beta-tubulin region of interest containing known mutation positions\nand predicted truncation"
  ) +
  theme_minimal()
